...
export type CatalogCompilerSources = {
  modules: Array<{ path: string; meta: MobileModuleMeta }>
  models: Array<{ path: string; config: MobileModelConfig }>
}
...
export function compileMobileRouteCatalog(sources: CatalogCompilerSources): MobileRouteCatalog {
  ...
  const modelsByKey = new Map<string, IndexedModelSource>()
  ...
  for (const modelSource of indexedModels) {
    ...
    modelsByKey.set(key, modelSource)
