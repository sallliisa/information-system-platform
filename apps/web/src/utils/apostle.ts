...
export type ApostleInit = RequestInit & { onUploadProgress?: (progress: { loaded: number; total: number }) => void }
export type ApostleInterceptor = (init: ApostleInit) => ApostleInit
...
export class Apostle {
  ...
  public async dispatch({ method, url, query, body, responseType, init }: ApostleRequestObject) {
    try {
      ...
      if (parsedInit.onUploadProgress) {
        delete (parsedInit as Partial<ApostleInit>).onUploadProgress
      }
      ...
      let finalBody: BodyInit | null = body as BodyInit
      ...
      if (onUploadProgress && body instanceof Blob) {
        ...
        finalBody = JSON.stringify(this.transformer.request(body as Record<string, string>))
      }

      const response = await fetch(`${url}?${qs.stringify(query as Record<string, string>)}`, {
        ...parsedInit,
  ...
  public async get(path: string | { path: string; query?: Record<string, string> }, responseType?: ApostleResponseType, init?: ApostleInit) {
    try {
  ...
  public async post(path: string | { path: string; query?: Record<string, string> }, body?: ApostleRequestBody, responseType?: ApostleResponseType, init?: ApostleInit) {
    try {
      const pathObject = typeof path === 'string' ? { path } : path
  ...
  public async put(path: string | { path: string; query?: Record<string, string> }, body?: ApostleRequestBody, responseType?: ApostleResponseType, init?: ApostleInit) {
    try {
      const pathObject = typeof path === 'string' ? { path } : path
  ...
  public async patch(path: string | { path: string; query?: Record<string, string> }, body?: ApostleRequestBody, responseType?: ApostleResponseType, init?: ApostleInit) {
    try {
      const pathObject = typeof path === 'string' ? { path } : path
  ...
  public async delete(path: string | { path: string; query?: Record<string, string> }, body?: ApostleRequestBody, responseType?: ApostleResponseType, init?: ApostleInit) {
    try {
      const pathObject = typeof path === 'string' ? { path } : path
