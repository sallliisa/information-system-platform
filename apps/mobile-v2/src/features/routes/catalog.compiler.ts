...
export const RESERVED_ROUTE_PATHS = new Set<string>([
  '/login',
  '/dashboard',
  '/dashboard/details',
  '/profile',
...
function parseModuleSourcePath(path: string): ParsedModuleSource {
  ...
  if (moduleFolderSlug !== moduleFileSlug) {
    throw new Error(`Module file name mismatch for "${path}".`)
  }
...
function parseModelSourcePath(path: string): ParsedModelSource {
  ...
  if (modelFolderSlug !== modelFileSlug) {
    throw new Error(`Model file name mismatch for "${path}".`)
  }
...
function buildCatalogEntry(moduleSlug: string, moduleMeta: MobileModuleMeta, modelSlug: string, config: MobileModelConfig): MobileCatalogEntry {
  return {
    ...
    hrefs: {
      list: `/${moduleSlug}/${modelSlug}/list`,
...
export function compileMobileRouteCatalog(sources: CatalogCompilerSources): MobileRouteCatalog {
  ...
  for (const modelSource of indexedModels) {
    ...
    if (modelSource.config.name !== modelSlug) {
      throw new Error(`Model name mismatch for ${modelSource.path}. Expected config.name "${modelSlug}".`)
    }

    const current = modelListByModule.get(moduleSlug)
    if (current) {
      const hasDuplicate = current.some((item) => item.parsed.modelSlug === modelSlug)
      if (hasDuplicate) {
        throw new Error(`Duplicate model definition for "${moduleSlug}/${modelSlug}".`)
      }
      current.push(modelSource)
    } else {
      modelListByModule.set(moduleSlug, [modelSource])
    }
  }
...
export function compileMobileRouteCatalogFromContexts(moduleContext: RuntimeContextModule, modelContext: RuntimeContextModule): MobileRouteCatalog {
