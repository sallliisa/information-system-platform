...
import { normalizeInternalRedirect } from './routes'
...
export async function getProfile<T = Record<string, unknown> | null>(): Promise<T> {
  return parseJSONValue<T>(await getStorageItem(PROFILE_KEY), null as T)
}

export async function setProfile(profile: Record<string, unknown>) {
...
export async function getPermissions<T = unknown[]>(): Promise<T> {
  return parseJSONValue<T>(await getStorageItem(PERMISSIONS_KEY), [] as T)
}

export async function setPermissions(permissions: unknown[]) {
...
export async function savePostLoginRedirect(path: string) {
  const normalizedPath = normalizeInternalRedirect(path)
...
export async function consumePostLoginRedirect(): Promise<string | null> {
  ...
  return normalizeInternalRedirect(storedPath)
}
