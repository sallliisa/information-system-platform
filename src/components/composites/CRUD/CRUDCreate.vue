...
import { bulkCreateFormProps, composeInputTemplateSheet } from '@/app/actions/CRUD/CRUDCreate'
...
const createFormConfig: CreateConfig = {
  fields: props.config.transaction?.create?.fields || props.config.transaction?.fields || props.config.fields || [],
  targetAPI: props.config.transaction?.create?.targetAPI || props.config.transaction?.targetAPI || props.config.name,
  fieldsAlias: { ...defaultFormConfig.fieldsAlias, ...(props.config.transaction?.create?.fieldsAlias || props.config.transaction?.fieldsAlias || props.config.fieldsAlias) },
